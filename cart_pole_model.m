...
%    alpha  = G(s) · Vm
%
%  Since G(s) is positive-gain (positive Vm → positive alpha),
%  and the PID is standard (positive error → positive output),
%  negative feedback is correct:
%
%    T(s) = G·C / (1 + G·C)      ← standard negative feedback
%
%  In MATLAB:  feedback(G*C, 1)   ← this applies negative feedback
%
%  NOTE: The previous version had feedback(-G*C, 1), which
%  actually gives POSITIVE feedback:  -G·C / (1 - G·C).
%  That's why the closed loop was unstable!
...
% Closed-loop (negative feedback — the correct form)
...
xlabel('Time [s]'); ylabel('\alpha / \alpha_{ref}');
...
title('Open-Loop Bode  L(s) = G \cdot C');
grid on;
...
title(sprintf('Nonlinear Cart-Pole: Pendulum Angle  (\\alpha_0 = %.0f°)', rad2deg(alpha0)));
legend('LQR-mapped PID', 'Tuned PID', 'Location', 'best');
grid on;
