...
struct SwingUpController {
    ...
    double compute(const QubeState& s) {
        ...
        // ── Energy calculation ─────────────────────────────────
        // Convention: E=0 at hanging (α=±π), E=E_ref at upright (α=0).
        // PE = mp·g·(Lp/2)·(1 + cos(α)):  0 at hanging, mp·g·Lp at upright.
        double hLp = p.half_Lp();
        double E = 0.5 * p.Jp * s.alpha_dot * s.alpha_dot
                 + p.mp * p.g * hLp * (1.0 + cos(s.alpha));

        // Target: energy at upright rest
        double E_ref = p.mp * p.g * hLp * 2.0;

        // ── Bang-bang: constant voltage, Åström direction ──────
        // u = mu · sign((E_ref - E) · α̇ · cos(α))
        // Energy flows motor→pendulum proportional to u·α̇·cos(α).
        // When E < E_ref: (E_ref-E) > 0 → u aligns with α̇·cos(α) → adds energy.
        // When E > E_ref: (E_ref-E) < 0 → u opposes α̇·cos(α) → removes energy.
        double direction = copysign(1.0, (E_ref - E) * s.alpha_dot * cos(s.alpha));
